...
const validateCred = (arr) => {
  ...
  const convertToNumArray = (str) => {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      if (/\d/.test(currChar)) {
        numArray.push(parseInt(currChar));
      }
    }
    return numArray;
  };
  ...
  const convertToValidCard = (arr) => {
    ...
    const sum = copyArr.reduce((acc, currValue) => acc + currValue);
    const remainder = sum % 10;
    if (remainder !== 0) {
      copyArr[copyArr.length - 1] += 10 - remainder;
    }
    return copyArr;
  };
  ...
  const findInvalidCards = (nestedArray) => {
    ...
    for (let i = 0; i < nestedArray.length; i++) {
      ...
      if (typeof currentCard === "string") {
        currentCard = convertToNumArray(currentCard);
      }
